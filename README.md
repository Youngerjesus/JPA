...
- JPA 에서 엔터티와 연관관계를 맺고 있는 엔터티를 지연 로딩으로 가지고올 때 프록시 객체를 통해서 가지고온다. 

- 프록시로 조회할 떈 PK 를 전달해준다. 
...
JPA 에서 즉시 로딩을 할때 기본적으로 INNER JOIN 이 아니라 LEFT OUTER JOIN 을 사용한다. 이는 데이터를 모두 다 조회하기 떄문에 성능상에 안좋을 수가 있다.

그러므로 INNER JOIN 을 사용해야 하는 경우라면 `@JoinColumn(nullable=flase` 로 설정하거나 `@ManyToOne(optional=false)` 로 설정하면 된다. 

컬렉션을 즉시 로딩으로 설정하는 경우에는 `optional` 설정과 관계 없이 무조건 Outer Join 을 한다. 

컬렉션을 지연 로딩 하는 경우에는 컬렉션 래퍼(PersistentBag) 으로 감싸는데 이게 프록시 역할을 해준다. 
...
SQL 조인 종류는 아니고 JPA 에서 성능 최적화를 위해 제공하는 기능으로 연관된 엔터티나 컬렉션을 SQL 한번에 함께 조회하는 기능이다.

패치 조인과 일반 조인의 차이는 일반 조인 실행시 연관된 엔터티를 함께 조회하지는 않는다. 그러므로 사용할때 계속해서 쿼리를 보내야한다. 

일반적인 Lazy Loading 은 프록시에서 필요한 시점에 값을 가져오도록 쿼리가 나간다. 내가 필요한 데이터가 연관관계에서 컬렉션으로 가지고 와야 하는 경우에

Lazy Loading 으로 하면 매번 쿼리가 나가는 문제가 생긴다. 이를 방지하기 위해서 패치 조인을 써서 한번에 값을 가지고 오도록 한다.     

지연 로딩으로 설정해도 패치 조인이 더 우선으로 적용된다. 

@ManyToOne 인 경우에는 패치 조인 @OneToMany 인 경우에는 컬렉션 패치 조인을 쓰면 된다.   

명령어는 join fetch 를 이용해 가능하다 

#### 패치 조인과 DISTINCT

SQL 의 DISTINCT 는 중복된 결과를 제거하는 명령을 내린다. 그치만 JPQL 은 또 다른 기능도 제공해주는데 이는 애플리케이션에서 엔터티 중복을 제거해준다.

@OneToMany 로 팀에서 멤버까지 조인해서 데이터를 가지고올때 패치 조인만 쓰면 팀에 있는 데이터가 뻥튀기 되는 문제가 생긴다. (DB 입장에서는 당연하지만)

이런 문제를 JPA 에서 DISTINCT 를 쓰면 어플리케이션 레벨로 데이터를 가지고 올 때 중복을 제거해준다. 
  

#### 패치 조인의 한계 

Fetch join 에서는 별칭을 줄 수 없다. 그리고 where 절을 넣어서 일부만 가지고 오도록 하는건 JPA 설계와 맞지 않는다. 

예상치 못한 동작을 할 수도 있기 떄문인데. 어떤 객체를 연관관계 조회할 때 다 가지고오도록 했는데 어떨때는 일부만 가지고오도록 한다면 
영속성 컨택스트는 어떻게 관리해야할지 모를 수가 있다. 필요하다면 별도의 쿼리르 만들어서 동작하도록 하자. 

둘 이상의 컬렉션은 패치 조인을 할 수 없다. 이는 1:N:N 이 되는데 데이터가 잘 안 맞는경우가 생길 수 있다. 

여러 테이블을 조인해서 엔터티가 가진 모양이 아닌 전혀 다른 결과를 내야한다면 패치 조인을 사용하 보다는 일반 조인을 사용해서 필요한 데이터들만 조회해가지고 DTO를 만들어서 전달해주는게 좋다. 

컬렉션을 패치 조인하면 페이징 API(setFistResult, setMaxResults) 를 할 수 없다. (일대다의 경우에 데이터가 뻥튀기 되므로
일대일 다대일은 가능하다)

컬렉션에서 페이징이 필요한 경우에는 패치 조인을 쓰지말고 `@OneToMany` 에 `BatchSize(size=100)` 옵션을 줘서 지정된 size 만큼 SQL 의 IN 절을 사용해서 조회하도록 한다. 

만약에 이 size 가 100 이라면 한번에 지연 로딩할떄 100개를 미리 로딩해두고 101번째 사용 시점에 다음 SQL 문을 추가로 실행한다.

`BatchSize` 를 사용할거라면 글로벌 하게 설정하도록 하자. 
